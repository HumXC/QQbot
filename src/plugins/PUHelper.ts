...
import { BotPlugin, BotPluginProfile, PluginUser, PluginUserManager } from "../lib/index";
export class Profile implements BotPluginProfile {
    Name: string = "PUHelper";
...
export class Plugin extends BotPlugin {
    private data = this.getData("data", new PUData());
    private users = new PluginUserManager(this.data);
    ...
    private config = this.getData("config", new PluginConfig());
    public init(): void {
        ...
        this.regKeyword("group", "group_admin", "^订阅pu", (message) => {
            let msg = "";
        ...
        this.regKeyword("group", "group_admin", "^退订pu", (message) => {
            let msg = "";
            let uid = Number((message as GroupMessageEvent).group_id);
            if (this.users.get(uid, "Group") === undefined) {
                msg = "本群已退订 PU 活动提醒";
            } else {
                this.users.rm(uid, "Group");
                this.config.save();
                msg = "本群已退订 PU 活动提醒";
            }
            message.reply(msg).catch((err) => {
        ...
        this.regKeyword("private", "friend", "^订阅pu", (message) => {
            let msg = "";
            let uid = Number((message as PrivateMessageEvent).sender.user_id);
            if (this.users.get(uid, "Person") !== undefined) {
                msg = "你已订阅 PU 活动提醒";
            } else {
                let u: PluginUser = {
                    uid: uid,
        ...
        this.regKeyword("private", "friend", "^退订pu", (message) => {
            let msg = "";
            let uid = Number((message as PrivateMessageEvent).sender.user_id);
            if (this.users.get(uid, "Person") === undefined) {
                msg = "你已退订 PU 活动提醒";
            } else {
                this.users.rm(uid, "Person");
                this.config.save();
                msg = "你已退订 PU 活动提醒";
            }
            message.reply(msg).catch((err) => {
