...
import { BotPlugin, BotPluginProfile, PluginUser, PluginUserManager, UserType } from "../lib/index";
...
interface User extends PluginUser {
    R18: boolean;
}
...
export class Plugin extends BotPlugin {
    ...
    private data = this.getData("data", new PluginData());
    private users = new PluginUserManager<User>(this.data);
    private plugin_user = (message: PrivateMessage | GroupMessage | DiscussMessage) => {
        let user;
        if (message.message_type === "group") {
            user = this.users.get(message.group_id, "Group");
        }
        if (message.message_type === "private") {
            user = this.users.get(message.sender.user_id, "Person");
        }
    ...
    public init(): void {
        ...
        this.regKeyword("global", this.plugin_user, "^来点.*色图$", (message) => {
        ...
        this.regKeyword("global", this.plugin_user, "获取信息", (message) => {
            let msg = "";
        ...
        this.regCommand(
            ...
            async (message, arg: string): Promise<boolean> => {
                let msg = "";
                let user: User = {
                    R18: false,
                    uid: 0,
                ...
                let _addUser = (uid: number, type: UserType): void => {
                    if (this.users.get(uid, "Group") === undefined) {
                        user.type = type;
                        user.uid = uid;
                        this.users.set(user);
                        this.data.save();
                ...
                let _rmUser = (uid: number, type: UserType): void => {
                    if (this.users.get(uid, "Group") === undefined) {
                        this.users.rm(uid, type);
                        this.data.save();
                    }
                    msg = "已关闭, 不可以色色!";
                };
                let _turnOnR18 = (uid: number, type: UserType): void => {
                    let u = this.users.get(uid, type);
                    if (u === undefined) {
                        msg = "爬";
                        return;
                    }
                ...
                let _turnOffR18 = (uid: number, type: UserType): void => {
                    let u = this.users.get(uid, type);
                    if (u === undefined) {
                        msg = "爬";
                        return;
                ...
                switch (arg) {
                    ...
                    case "开启R18":
                        switch (message.message_type) {
                            case "group":
                                _turnOnR18(message.group_id, "Group");
                                break;
                            case "private":
                                _turnOnR18(message.sender.user_id, "Person");
                                break;
                            default:
                                return true;
                        }
                    ...
                    case "关闭R18":
                        switch (message.message_type) {
                            case "group":
                                _turnOffR18(message.group_id, "Group");
                                break;
                            case "private":
                                _turnOffR18(message.sender.user_id, "Person");
                                break;
                            default:
                                return true;
                        }
    ...
    getSendedSetu(messsageSource: Quotable): Setu | null {
        for (let i = 0; i < this.sendedSetu.length; i++) {
            ...
            if (
                ...
                messsageSource.user_id == this.client.oicq.uin
            ) {
