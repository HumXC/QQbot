...
import { BotPlugin, BotPluginProfile, PluginUser } from "../lib/index";
...
export class Plugin extends BotPlugin {
    public init() {
        this.client.on("newday", () => {
            setTimeout(() => {
                this.logger.info("清除图片信息缓存");
                allImage.clear();
            }, 14400000);
        });
        this.client.oicq.on("message", (m) => {
        ...
        this.regKeyword("global", "allow_all", "搜图", async (message) => {
