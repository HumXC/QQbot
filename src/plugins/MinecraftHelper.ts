...
export class Plugin extends BotPlugin {
    ...
    private users = new PluginUserManager<User>(this.config);
    ...
    public init() {
        this.regKeyword("global", this.plugin_user, "^查服", async (message) => {
        ...
        this.regKeyword("global", this.plugin_user, "mc", async (message) => {
            let user: User | undefined;
            let sendstr = "";
            switch (message.message_type) {
                case "group":
    ...
    private getUser(uid: number, type: UserType): User | undefined {
        if (type === "Group") {
            return this.users.get(uid, "Group");
        } else {
            return this.users.get(uid, "Person");
        }
    }
